...
$(function() {
  ...
  function set_settings() {
    ...
    settings.defaultredirect = 'https://yasarpsych.qualtrics.com/jfe/form/SV_1Ybypkf87Y1iCt8';
    ...
    settings.condition_1_likes = [13000, 9999999]; 
    ...
    settings.likes_by = ['Aynurella','AncaD','EsraT','Arjen','Jane','Tahsinnn','Lidya','Kenaan','Zeynep']; 
  }
  ...
  function init_name() {
  	...
  	$('#submit_username').on('click',function() {
  		...
  		if(not_alphanumeric(uname)) {
  			error = 1;
  			errormsg = 'Sadece harf giriniz (boşluk olmadan)';
  		}  		
  ...
  function init_text() {
  	...
  	$('#submit_text').on('click',function() {
  		...
  		if($('#description').val().length > 401) {
  			...
  			error = 1;
  			errormsg = 'Kelime sınırı aşıldı lütfen kelime sayısını azaltın';
  		}  		
  		if(error == 0) {
  			$('#text').hide();
